...
Create a new ADR(Architectural decission record) in adr/*.md to resolve the `Task` using the exact specified markdown `Plan Format`. Follow the `Instructions` to create the plan use the `Relevant Files` to focus on the right files. Follow the `Report` section to properly report the results of your work.

## Instructions

- IMPORTANT: You're writing an ADR to resolve a Tasl based on the `Task` that will provide the fundational architectural rules for the application.
- IMPORTANT: The `Task` describes the Task that will be resolved by this architectural approach, remember we're not resolving the Task, we're creating the ADR that will be used to resolve the Task based on the `Plan Format` below.
- You're writing a ADR to resolve a tasl, it should be simple but we need to be thorough and precise so we don't miss anything or waste time with any second round of changes.
...
- When you finish creating the adr for the tasl, follow the `Report` section to properly report the results of your work.
